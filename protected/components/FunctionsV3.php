...
class FunctionsV3
{
    ...
    public static function getMenu($class="menu")
    {
		...
		$top_menu[]=array('tag'=>"orderHistory",
		'visible'=>in_array('browse',(array)$top_menu_activated)?true:false,
		...
		'visible'=>in_array('browse',(array)$top_menu_activated)?true:false,
		'label'=>'<span class="brun-user-profile brunicon"></span>',
    ...
    //             'url'=>array('/store/signup'));   
        ...
        if ( Yii::app()->functions->isClientLogin()){
        	$top_menu[]=array(
        	  ...
        	  'itemOptions'=>array('class'=>'green-button f-hide')      	  
        	);
        	
        	$top_menu[]=array(
        	  'url'=>array('/store/logout'),
        	  'label'=>t("Sign Out"),
        	  'itemOptions'=>array('class'=>'logout-menu orange-button f-hide')
        	);
        }
...
/*	public static function getItemFirstPrice($prices='',$discount='')
    ...
    public static function receiptRowTotal($label='',$value='',$class1='',$class2='')
    {
    	$html='';
    	$html.="<div class=\"row\">";
    	$html.="<div class=\"col-md-6 col-xs-6 $class1\">".t($label)."</div>";
	...
	public static function sendPartnerSignupEmail($data='')
    {
		...
		if(!empty($tpl)){
			...
			$tpl=self::smarty('partner_phone',
			isset($data['partner_phone'])?$data['partner_phone']:"",$tpl);

			$tpl=self::smarty('partner_email',
			isset($data['partner_email'])?$data['partner_email']:"",$tpl);

			$tpl=self::smarty('partner_rest_name',
			isset($data['partner_rest_name'])?$data['partner_rest_name']:"",$tpl);

			$tpl=self::smarty('partner_rest_phone',
			isset($data['partner_rest_phone'])?$data['partner_rest_phone']:"",$tpl);

			$tpl=self::smarty('partner_rest_city',
			isset($data['partner_rest_city'])?$data['partner_rest_city']:"",$tpl);

			$tpl=self::smarty('partner_rest_address',
			isset($data['partner_rest_address'])?$data['partner_rest_address']:"",$tpl);

			$tpl=self::smarty('partner_rest_state',
			isset($data['partner_rest_state'])?$data['partner_rest_state']:"",$tpl);

			$tpl=self::smarty('partner_rest_post_code',
			isset($data['partner_rest_post_code'])?$data['partner_rest_post_code']:"",$tpl);

			$tpl=self::smarty('partner_rest_country',
			isset($data['partner_rest_country'])?$data['partner_rest_country']:"",$tpl);
			
			$tpl=self::smarty('partner_rest_website',
			isset($data['partner_rest_website'])?$data['partner_rest_website']:"",$tpl);
   ...
   public static function getDashboardLinkImage($logo='')
   {    	
	   $upload_path=Yii::getPathOfAlias('webroot')."/upload";     
	   if (!empty($logo)){
		   if (file_exists($upload_path."/".$logo)){
