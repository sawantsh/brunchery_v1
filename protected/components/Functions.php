...
{	
	...
	public function searchRestaurants()
	{
		...
		$filter_delivery='';$filter_delivery_arr=array();
		if (isset($_GET['filter_delivery_type'])){			
			$filter_delivery_type=$_GET['filter_delivery_type'];
			if (is_array($filter_delivery_type) && count($filter_delivery_type)>=1){
				foreach ($filter_delivery_type as $val) {
					if (!empty($val)){
						$filter_delivery.="'$val',";
		...
		$filter_cuisine='';
		if (isset($_GET['filter_cuisine'])){
			$filter_cuisines=$_GET['filter_cuisine'];
			if (is_array($filter_cuisines) && count($filter_cuisines)>=1){
				$x=1;
				foreach ($filter_cuisines as $val) {				
					if (!empty($val)){
	...
	public function displayOrderHTML($data='',$cart_item='',$receipt=false,$new_order_id='')
	{
    	...
    	if (isset($cart_item)){
    		if (is_array($cart_item) && count($cart_item)>=1){
				$x=0;
				$total_cart_items = 0;
				$htm.='<!-- ordered-items --><div class="ordered-items"><h4 class="rgt-border">Your Order</h4>';
    			foreach ($cart_item as $key=>$val) {	    				
					...
					$total_cart_items = $total_cart_items + $qty;
					$total_price=$val['qty']*$price;
					$this->msg=Yii::t("default","" + $qty + "" + $total_price);
    				...
    				//   } 
			          ...
			          $htm.='<div class="b col-md-10">'.qTranslate(
					  ...
					  $htm.='<div class="a col-md-1"> x'.$val['qty'].'</div>';
			          
			          $htm.='<div class="manage col-md-1">';
			            $htm.='<div class="c f-hide">';
			          ...
			          $htm.='</div>';
			          $htm.='<div class="clear"></div><!--item-order-list item-row -->';
...
'sub_item_name',$addon_name_trans).'</div>';
				...
				$htm.="<div class=\"store-cart-summary\"><a href=\"#\" class=\"edit-order-pop\"><div class=\"cart-icon icon center-cropped\"><i class=\"ion-ios-cart\"></i><span style=\"color:white\" class=\"item-count\">$total_cart_items</span></div></a>";
		       ...
		       $htm.="<div class=\"row cart_total_wrap\">";
	    	   $htm.="<div class=\"col-md-6 col-xs-6 \">".t("Total")."</div>";
	    	   $htm.="<div class=\"col-md-6 col-xs-6  text-right cart_total\"> To be paid <b>".
	    	   displayPrice(baseCurrency(),prettyFormat($total,$mid))."</b></div>";
	    	   $htm.="</div>";
			   ...
			   $htm.=CHtml::hiddenField("total_price",unPrettyPrice($total));
...
$menu_html.="</li>";
   ...
   {    	
	...
	public function GetDashboardLink($id='')
	{
		$DbExt=new DbExt;
		$stmt="SELECT * FROM
		{{dashboard_links}}
