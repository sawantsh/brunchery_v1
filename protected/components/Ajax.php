...
$this->msg=t("We have sent bank information instruction to your email")." :$merchant_email";
	...
	public function dashboardLinks()
	{
	    ...
	    if (is_array($rows) && count($rows)>=1){
	    	foreach ($rows as $val) {    	     	    		
	    		$chk="<input type=\"checkbox\" name=\"row[]\" value=\"$val[link_id]\" class=\"chk_child\" >";   		
	    			    		
	    		$slug=Yii::app()->createUrl('/admin/dashboardLinks',array(
	    		  'Do'=>"Add",
	    		  'id'=>$val['link_id']
	    		));
	    		...
	    		$option="<div class=\"options\">
	    		<a href=\"$slug\" >".Yii::t("default","Edit")."</a>
	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[link_id]\" >".Yii::t("default","Delete")."</a>
	    		</div>";
	...
	public function addDashboardLink()
	{		
	   $Validator=new Validator;
		$req=array(
		  'link'=>Yii::t("default","Link is required"),
		  'spicydish'=>t("Icon is required")
		);		
		$Validator->required($req,$this->data);
		if ($Validator->validate()){
		   ...
		   if (empty($this->data['id'])){	
		    	if ( $this->insertData("{{dashboard_links}}",$params)){
		    		    $this->details=Yii::app()->db->getLastInsertID();
			    		$this->code=1;
			    		$this->msg=Yii::t("default","Successful");				    		
			    	}
					...
					$filename_to_delete='';
					if($old_data=Yii::app()->functions->GetDashboardLink($this->data['id'])){
						if($old_data['photo']!=$this->data['spicydish']){
						   $filename_to_delete=$old_data['photo'];			
						}			
					}
					
					$res = $this->updateData('{{dashboard_links}}' , $params ,'link_id',$this->data['id']);
					if ($res){
						$this->code=1;
		                $this->msg=Yii::t("default",'Link updated');  
