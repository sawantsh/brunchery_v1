...
CREATE TABLE IF NOT EXISTS ".$table_prefix."dashboard_links (
  `link_id` int(14) NOT NULL AUTO_INCREMENT,
...
";
echo "Creating Table dashboard_links..<br/>";	
$DbExt->qry($stmt);
echo "(Done)<br/>";
...
echo "(Done)<br/>";
	  ...
	  MODIFY `id` int(14) NOT NULL AUTO_INCREMENT;
		...
		/*dashboard_links*/
		$this->addIndex("dashboard_links","link");
		...
		echo "Running Bruncherry specific updates now...<br/>";

		$stmt = "update ".$prefix."option 
				set option_value = '2' 
				where option_name in (
					'customer_ask_address', 
					'theme_hide_logo',
					'theme_hide_how_works',
					'theme_hide_cuisine',
					'theme_search_merchant_name',
					'theme_search_cuisine',
					'theme_search_foodname',
					'theme_hide_footer_section1',
					'theme_hide_footer_section2',
					'merchant_tbl_book_disabled',
					'theme_map_tab',
					'theme_photos_tab',
					'theme_time_pick',
					'admin_menu_allowed_merchant'
				)";
		echo "Updating Options with 2 value..<br/>";	
		$DbExt->qry($stmt);
		echo "(Done)<br/>";	
		...
		echo "Updating Options with yes value..<br/>";	
		$DbExt->qry($stmt);
		echo "(Done)<br/>";
		...
		echo "Updating Options with 1 value..<br/>";	
		$DbExt->qry($stmt);
		echo "(Done)<br/>";
		...
		$stmt = "update ".$prefix."option 
				set option_value = '' 
				where option_name in (
					'admin_activated_menu'
				)";
		echo "Updating Options with empty value..<br/>";	
		$DbExt->qry($stmt);
		echo "(Done)<br/>";
