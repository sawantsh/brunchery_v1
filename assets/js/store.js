...
jQuery(document).ready(function() {	
   ...
   $( document ).on( "click", ".checkout", function() {    	
	 ...
	 var subtotal= parseFloat($("#subtotal_order").val());
   	 ...
   	 if ( $("#delivery_type").val()=="pickup"){   	     
		 ...
		 window.location.href=sites_url+"/checkout?redirect=paymentoption";
   ...
   $( document ).on( "click", ".place_order", function() {    	  
	...
	if (!(arrival_time_hr >= 0 && arrival_time_hr <= 23 
		&& arrival_time_min >= 0 && arrival_time_min <= 59)) {
...
{	
    ...
    success: function(data){ 
    	...
    	if (data.code==1){
			...
			$(".confirm-cart_total").html("$"+data.details.raw.total.total);
			$(".voucher_wrap").show();
			...
			$(".cart-icon").addClass("disabled-button")
    		$(".voucher_wrap").hide();
    		clearCartButton(2);
    	}
    }, 
